...
(function() {
    ...
    const IDLE_THRESHOLD = 4.0; // Adjusted to 4 seconds based on decision fatigue research
    ...
    // --- RESTORED DUAL-DOT DASHBOARD & TELEMETRY ---
    ...
    function startCountdown() {
        statusText.innerText = `Idle: ${countdownValue.toFixed(1)}s`;
        ...
        countdownInterval = setInterval(() => {
            ...
            if (countdownValue <= 0) {
                ...
                // Dual-key condition: Spotlight triggers ONLY if idle AND overloaded
    ...
    chrome.runtime.onMessage.addListener((message) => {
        if (message.type === 'TEPR_TELEMETRY') {
            ...
            if (message.fatigue === 'CAM' || message.fatigue === 'MDL') {
                telemetryText.style.color = "#ff4a4a";
                emotionDot.style.background = "red";
                emotionDot.style.boxShadow = "0 0 8px red";
            } else if (message.isSpike) {
                telemetryText.style.color = "#eab308";
            } else {
                telemetryText.style.color = "#4ade80";
            }
        }
        ...
        if (message.type === 'TEPR_SPIKE') {
            ...
            // If the user was already idle when the spike occurred, trigger immediately
    ...
    function triggerSpotlightAt(x, y) {
        ...
        // Ascend the DOM to find a meaningful semantic block of content
